...
Add the real MoMo Collection API values to `backend/.env`. The backend accepts the collection key as `MOMO_COLLECTION_SUBSCRIPTION_KEY` (or legacy `MOMO_SUBSCRIPTION_KEY`). Credentials are intentionally not included in tracked project files. Payment requests are created by the backend and the order is confirmed only after the provider reports `SUCCESSFUL`.
